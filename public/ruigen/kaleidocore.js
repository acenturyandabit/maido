			...
			for (var k=0;k< v.data.shapes.length;k++) {
				...
				for (var i = 0; i < s.factor; i++) {
					...
					for (var j=0;j<s.bits.length;j++) {
						b=s.bits[j];
						xx = (s.r) * Math.cos(s.t + i * Math.PI * 2 / s.factor) + b.dr * Math.cos(b.dt + i * Math.PI * 2 / s.factor) + v.e.width / 2;
						yy = (s.r) * Math.sin(s.t + i * Math.PI * 2 / s.factor) + b.dr * Math.sin(b.dt + i * Math.PI * 2 / s.factor) + v.e.height / 2
						if (ist) {
							v.ctx.moveTo(xx, yy);
							ist = false;
