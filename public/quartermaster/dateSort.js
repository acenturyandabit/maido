function addDateSorter(quarterMaster) {
    ...
    quarterMaster.dateParse = function (item) {
        ...
        for (let k = 0; k < dvchain.length; k++) {
            ...
            for (let z = 0; z < quarterMaster.dateParserRegexes.length; z++) {
                ...
                while ((regres = quarterMaster.dateParserRegexes[z].regex.exec(dv)) != null) {
                    seen = true;
    ...
    `, document.body, "input[data-role='date']", (e) => {
            dateContextedTarget = quarterMaster.items[e.currentTarget.parentElement.querySelector("[data-role='id']").value];
            //find currentTarget
        });
        document.getElementById("rectify").addEventListener("click",()=>{
            dateContextedTarget.dateString=(new Date(dateContextedTarget.dates[0])).toLocaleString();
